...
public class DelCommand implements CommandExecutor {
    public boolean onCommand(final CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length == 0) {
            sender.sendMessage(TextPrefix.PrefixedMessage("Please specify a block to allow snow to form on"));
        } else {
            ...
            List<Material> removedBlocks = new ArrayList<>();
            ...
            removedBlocks.forEach(blockType -> blockTypesString.append(blockType.name()).append(","));
            // trim comma off end of string
            if (blockTypesString.length() > 0) {
                blockTypesString.setLength(blockTypesString.length() - 1);
            } else {
                sender.sendMessage(TextPrefix.PrefixedMessage("No blocks removed"));
                return true;
            }

            sender.sendMessage(
                    TextPrefix.PrefixedMessage("Removed " + blockTypesString
                            + " from the list of blocks that snow will not form on"));
        }
        return true;
    }
